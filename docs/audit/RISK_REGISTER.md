...
| **R-07** | **Tennis vendor-identity ambiguity.** `api-tennis.com` vs `tennis-api.com` (matchstat) are similarly named, possibly distinct vendors. Wiring the wrong one wastes effort / mis-maps data. | Med | Med | Confirm exact vendor + auth + base URL against a real key before writing the adapter; record in an ADR. | tennis-data-agent |
